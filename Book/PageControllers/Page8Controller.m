...
-(void) playSound {
    soundPath = [[NSBundle mainBundle] pathForResource:@"page8" ofType:@"wav"];
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    NSMutableAttributedString *label = [[NSMutableAttributedString alloc] initWithString:@"Koodles loves to climb into his big fluffy bed with his favorite stuffed bunny rabbit."];
    
    NSInteger stringLength = [label length];
    
    //Sets font. Notice: NSMakeRange(startingIndex, lengthofChars)
    UIFont *font = [UIFont fontWithName:@"SnellRoundhand-Bold" size:48.0];
...
- (void)setupTimers{
    ...
    NSDictionary *currentPage = [dict objectForKey:@"Page8"];
