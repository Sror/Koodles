...
-(void) playSound {
    soundPath = [[NSBundle mainBundle] pathForResource:@"page5" ofType:@"wav"];
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    NSMutableAttributedString *label = [[NSMutableAttributedString alloc] initWithString:@"He also likes spending time with his friends. They play games and sports together."];
    
    NSInteger stringLength = [label length];
    
    //Sets font. Notice: NSMakeRange(startingIndex, lengthofChars)
    UIFont *font = [UIFont fontWithName:@"Papyrus-Condensed" size:45.0];
...
- (void)setupTimers{
    ...
    NSDictionary *currentPage = [dict objectForKey:@"Page5"];
    ...
    [NSTimer scheduledTimerWithTimeInterval:7.6
                                     target:self
